...
// Instance method to check if user liked the reel
...
// Static method to get explore reels
reelSchema.statics.getExploreReels = function(userId, page = 1, limit = 20) {
  ...
  return this.aggregate([
    ...
    { $unwind: '$authorInfo' },
    // Filter out reels from private accounts that user doesn't follow
    {
      $match: {
        $or: [
          { 'authorInfo.isPrivate': false }, // Only public reels in explore
          { 
            $and: [
              { 'authorInfo.isPrivate': true },
              { 'authorInfo.followers': { $in: [new mongoose.Types.ObjectId(userId || '000000000000000000000000')] } }
            ]
          } // Private reels from followed users (if userId exists)
        ]
      }
    },
    {
      $addFields: {
        ...
        isLikedBy: {
          $cond: {
            if: { $eq: [userId, null] },
            then: false,
            else: { $in: [new mongoose.Types.ObjectId(userId), '$likes.user'] }
          }
        }
      }
    },
...
reelSchema.statics.getUserReels = function(userId, page = 1, limit = 12, requestingUserId = null) {
  const skip = (page - 1) * limit;
  ...
  return this.aggregate([
    ...
    },
    {
      $addFields: {
        isLikedBy: {
          $cond: {
            if: { $eq: [requestingUserId, null] },
            then: false,
            else: { $in: [new mongoose.Types.ObjectId(requestingUserId), '$likes.user'] }
          }
        }
      }
    },
        ...
        pipeline: [
          {
            $match: {
              $expr: {
                $and: [
                  { $eq: ['$_id', new mongoose.Types.ObjectId(requestingUserId || '000000000000000000000000')] },
                  { $in: ['$$reelId', '$savedReels'] }
                ]
              }
            }
